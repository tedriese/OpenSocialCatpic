...
/**
 * @fileoverview Runner of the tests.  It's used by various unit test gadgets
 * running from various containers such as orkut and igoogle.  The runner
 * first obtains the context (the gadget's viewer, owner and container).  The
 * context determines the strategy used for verifying the test results.  The
 * runner then instantiates the validator which loops thru the tests inside a
 * given test suite, and calls each tests run method.  Upon finishing, the
 * validator calls TestRunner's renderTestOutput method to print out the test
 * results.
 */
 ...
 *     test.
 * @param {opt_sequential} boolean value indicating if tests will be run
 *  sequentially.
 * @constructor
 */
function OpenSocialTestRunner(object, opt_sequential) {
  /**
  ...
  /**
   * Json object that stores the golden result for this directory
   * @type {Object}
   */
  this.goldenResults = null;
  ...
  /**
   * flag to tell test runner if tests should be executed sequentially
   */
  this.sequential = opt_sequential;
 ...
 * Inherit testRunner
 */
OpenSocialTestRunner.prototype = new TestRunner();
 ...
 * @override
 */
OpenSocialTestRunner.prototype.getContext = function() {
  return this.context;
};
 ...
 * @override
 */
OpenSocialTestRunner.prototype.getGoldenResults = function() {
 ...
 * @override
 */
OpenSocialTestRunner.prototype.onAllFinish = function(resultSets) {
  ...
  runner.add(this.logger.updateResultBoxes,
      [resultSets, this.goldenResults, this.context], this.logger);
  runner.run();
};
 ...
 * @override
 */
OpenSocialTestRunner.prototype.onSuiteFinish = function(resultSets) {
  var runner = new Helper.FunctionRunner();
 ...
 * @override
 */
OpenSocialTestRunner.prototype.onTestFinish = function(result, test) {
  var runner = new Helper.FunctionRunner();
 ...
 * @override
 */
OpenSocialTestRunner.prototype.setupTests = function(finishCallback) {
  ...
  req.send(function(dataResponse) {
    ...
    that.renderTestGrid(context.getInfo());
    finishCallback();
  });
};
 ...
 */
OpenSocialTestRunner.prototype.executeTests = function() {
  var validator;
  if (this.sequential) {
    validator = new SequentialValidator(
        this.getContext(), this.testSuiteDirectory);
  } else {
    validator = new Validator(this.getContext(), this.testSuiteDirectory);
  }
 ...
 */
OpenSocialTestRunner.prototype.setSingleTestAsDirectory = function(testId) {
 ...
 */
TestRunner.prototype.reRunSuite = function(suiteId) {
 ...
 */
OpenSocialTestRunner.prototype.setSingleSuiteAsDirectory = function(suiteId) {
  var directory = this.testSuiteDirectory;
  for (var i = 0; i < directory.suites.length; i++) {
